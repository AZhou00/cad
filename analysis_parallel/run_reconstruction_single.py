...
Diagonal covariance approximation on scan-observed pixels:
  ...
  with Ntilde_s = N_s + W_s C_a W_s^T.
  Approximate Ntilde_s as sample-diagonal:
    diag(F_s)_j ~= sum_{i in samples hitting pixel j} 1 / var_eff_i
    var_eff_i = sigma_i^2 + (W_s C_a W_s^T)_{ii}
  therefore:
...
def _atm_var_per_sample_from_w4(*, prior_atm: cad.FourierGaussianPrior, w4: np.ndarray) -> np.ndarray:
    """
    Approximate diag(W C_a W^T) for bilinear W using stationary C_a.

    For each valid sample i:
      (W C_a W^T)_{ii} = w_i^T K4 w_i,
    where K4 is the 4x4 covariance matrix of bilinear corner offsets
    [(0,0), (1,0), (0,1), (1,1)] under the stationary atmosphere prior.
    """
...
def run_one_scan(
    ...
    *,
    ...
    # Diagonal precision approximation from sample-diagonal Ntilde.
